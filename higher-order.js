...
let myStrongest = monstersInYourPocket.filter((el) => (el['CP'] > 199))
...
let ordersTotal = orders.reduce((acc, val) => {
    acc['price'] = acc['price'] * (1 + acc['tax'])
    val['price'] = val['price'] * (1 + val['tax'])
  return orders;
  }
)
...
let bobsTotal = purchases.filter((el) => el['owner'] === 'Bob').reduce(((acc, val) => acc + val['price']), 0)
